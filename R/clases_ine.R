...
DiseñoINE <- R6::R6Class(
  ...
  public = list(
    ...
    agregar_nivel = function(variable, tipo, descripcion, llave) {
      if (!tipo %in% c("strata", "cluster")) {
        stop("Tipo debe ser igual a cluster o strata")
      }
      ...
      self$poblacion$marco_muestral <- self$poblacion$marco_muestral %>%
        {
          if (self$ultimo_nivel != 0) {
            agrupar_nivel(., nivel = self$ultimo_nivel)
          } else {
            .
          }
        } %>%
    ...
    eliminar_nivel = function(nivel) {
      ...
      self$n_i <- self$n_i[
        -grep(glue::glue("(cluster|strata)_[{nivel}-9]"), names(self$n_i))
      ]
    },
    plan_muestra = function(nivel, criterio, unidades_nivel, manual) {
      ...
      if (nivel_l == 0) {
        ...
        res <- self$poblacion$marco_muestral %>%
          group_by(cluster_0) %>%
          summarise(m_0 = self$n_0, n_0 = self$n_0)
    ...
    sobremuestra = function(nivel, unidad, unidad_elegida, total_m, total_n) {
      ...
      tipo_nivel <- self$n_i %>%
        names() %>%
        grep(glue::glue("strata|cluster_{nivel}"), ., value = T)
        ...
        self$sobre_muestra <- self$sobre_muestra %>%
          bind_rows(
            modif %>%
              ...
              mutate(
                !!rlang::sym(unidad) := !!unidad_elegida,
                m_sm = round(total_m * m_1_vieja / sum(m_1_vieja)),
                n_sm = total_n * n_1_vieja / sum(n_1_vieja)
              )
          )
      } else {
    ...
    extraer_muestra = function(nivel) {
      ...
      if (nivel == self$ultimo_nivel) {
        #actualizar fpc_0
        if (
          self$niveles %>% filter(nivel == 0) %>% pull(unidades) != nrow(aux)
        ) {
          ajuste <- ((self$niveles %>% filter(nivel == 0) %>% pull(unidades)) -
            nrow(aux)) *
            self$n_0
    ...
    sustituir_muestra = function(
      ...
      self <- sustituir_muestra_ine(
        ...
        id,
        zoom,
        dir = glue::glue("{self$dir.exportar}/Mapas"),
        ajustar_cuotas = ajustar_cuotas,
        crear_mapa = crear_mapa
      )

      if (crear_mapa) {
        if (!file.exists(glue::glue("{self$dir.exportar}/Mapas/Eliminadas"))) {
          dir.create(glue::glue("{self$dir.exportar}/Mapas/Eliminadas"))
        }

        file.rename(
          glue::glue("{self$dir.exportar}/Mapas/{id}.png"),
          glue::glue("{self$dir.exportar}/Mapas/Eliminadas/{id}.png")
        )

        readr::write_rds(
          self,
          glue::glue("{self$dir.exportar}/diseño{self$n_sustitucion}.rda")
        )
        self$cuotas %>%
          readr::write_excel_csv(glue::glue(
            "{self$dir.exportar}/cuotas{self$n_sustitucion}.csv"
          ))
      }
...
PoblacionINE <- R6::R6Class(
  ...
  public = list(
    ...
    initialize = function(nombre, ln, electoral, shp_mza, shp_loc, shp_mun) {
      self$nombre = nombre

      self$marco_muestral = muestreaR::crear_mm_ine(
        ln = ln,
        shp_mza = shp_mza,
        shp_loc = shp_loc,
        shp_mun = shp_mun
      ...
      self$informacion_electoral <- electoral %>%
...
CartografiaINE <- R6::R6Class(
  ...
  public = list(
    ...
    graficar_mapa = function(lflt = NULL, bd, nivel) {
      ...
      graficar_mapa_muestra_ine(
        lflt = lflt,
        muestra = if (!is.data.frame(bd)) {
          bd %>% purrr::pluck(nivel_p) %>% tidyr::unnest(data)
        } else {
          bd
        },
        shp = self$shp,
    ...
    crear_mapas = function(diseño, zoom, dir, contexto_m = NULL,
      ...
      google_maps_ine(diseño, shp = self$shp, zoom = zoom, dir = dir,
                      ...
                      etiquetas_unidad = etiquetas_unidad)
    }
  )
)
